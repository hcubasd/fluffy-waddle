...
def assemble_pipeline_stage_map(rows, pipeline_map):
    try:
        return {
            r["id"]: CRMPipelineStage(**{**r, "pipeline": pipeline_map[r["pipeline_id"]]})
                for r in rows
        }
    except Exception as e:
    ...
    try:
        ...
        for r in org_industries_rows:
            if r["industry_id"] in industries_map:
                org_industries.setdefault(r["organization_id"], []).append(industries_map[r["industry_id"]])

        org_followers: dict[str, list] = {}
        for r in org_followers_rows:
            if r["user_id"] in users_map:
                org_followers.setdefault(r["organization_id"], []).append(users_map[r["user_id"]])

        org_contacts: dict[str, list] = {}
        for r in contact_rows:
            if r.get("organization_id") and r["id"] in contact_map:
                org_contacts.setdefault(r["organization_id"], []).append(contact_map[r["id"]])
...
def assemble_deal_contacts_map(deals_contacts_rows, contacts_map):
    ...
    for r in deals_contacts_rows:
        if r["contact_id"] in contacts_map:
            deal_contacts.setdefault(r["deal_id"], []).append(contacts_map[r["contact_id"]])
...
def assemble_deal_products_map(deals_products_rows, products_map):
    ...
    for r in deals_products_rows:
        if r["product_id"] in products_map:
            deal_products.setdefault(r["deal_id"], []).append(products_map[r["product_id"]])
...
def assemble_tasks_by_deal_map(tasks_rows, users_map, tasks_users_rows):
    try:
        ...
        for r in tasks_users_rows:
            if r["user_id"] in users_map:
                task_assignees.setdefault(r["task_id"], []).append(users_map[r["user_id"]])
        ...
        for r in tasks_rows:
            t = CRMTask(**{
                **r,
                ...
                "completed_by": users_map.get(r["completed_by_id"]) if r["completed_by_id"] else None,
        ...
        deals = [
            CRMDeal(**{
                **r,
                ...
                "owner": users_map.get(r["owner_id"]) if r["owner_id"] else None,
                "source": sources_map.get(r["source_id"]) if r["source_id"] else None,
                "campaign": campaigns_map.get(r["campaign_id"]) if r["campaign_id"] else None,
                "loss_reason": loss_reasons_map.get(r["loss_reason_id"]) if r["loss_reason_id"] else None,
                "organization": organizations_map.get(r["organization_id"]) if r["organization_id"] else None,
